...
class ArithmeticalFeatures_1(BasePreprocessor):
    """
    ...
    def _parallel_transform(
        ...
        n_samples, _ = X.shape
        if self.n_features_ > n_samples:
            return X

        n_operands = len(self._operand)
        n_combinations = self.n_features_ * (self.n_features_ - 1) // 2

        if n_operands * n_combinations < self.max_n_combinations:
            Xt = np.empty((n_samples, n_operands * n_combinations), dtype=dtype)
        else:
            Xt = np.empty((n_samples, self.max_n_combinations), dtype=dtype)

        break_point = 0

        for i, operand in enumerate(self._operand):
            iterable = itertools.combinations(b, 2)
            ...
            for j, (k, l) in enumerate(iterable):
                ...
                break_point += 1
                if break_point == self.max_n_combinations:
                    break
