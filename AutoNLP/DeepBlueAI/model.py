...
def CNN_Model(seq_len, num_classes, num_features, embedding_matrix=None):
    ...
    if embedding_matrix is None:
        ...
        x = Embedding(
            num_features, 300, trainable=trainable, weights=[embedding_matrix]
        )(in_text)
...
class Model(object):
    ...
    def train(self, train_dataset, remaining_time_budget=None):
        ...
        if self.START:
            ...
            if len_mean > self.FIRST_CUT:
                print(
                    "len mean {} FIRST CUT {} need cut.".format(
                        len_mean, self.FIRST_CUT
                    )
                )
                cut = 1
        ...
        if self.START or self.FIRSTROUND or self.LASTROUND:
            ...
            print(
                "\n--- Start Train: \n\tdata_id {} \n\tmodel_id {} \n\tdone_training {} \n\tepoch {}".format(
                    self.data_id, self.model_id, self.done_training, self.epoch
                )
            )
            models = ["CNN", "GRU", "", "", "", ""]
            methods = [
        ...
        if len(y_train) > 10000:
            if self.epoch < 4:
                batch_size = (
                    (6 - self.epoch) * 32 * int(len(self.X_train) / 10000)
                )  # 256
            else:
    ...
    def deal_data(self, data, data_lan, data_type, deal_seg, sentence_len):
        ...
        if data_type == 0:
            ...
            res = ac.texts_to_sequences_and_pad(
                t, num_sentence, word2index, max_length, text_lens, data_type
            )
            ...
            max_length = self.max_length
            res = ac.texts_to_sequences_and_pad(
                t, num_sentence, word2index, max_length, None, data_type
            )
            return res
    ...
    def get_embedding(self, num_features, word_index, version="official"):
        ...
        if version == "RENMIN":
            fasttext_embeddings_index = (
                GET_EMBEDDING_NEW.fasttext_embeddings_index_zh
                if self.metadata["language"] == "ZH"
                else GET_EMBEDDING.fasttext_embeddings_index_en
            )
        elif version == "official":
            fasttext_embeddings_index = (
                GET_EMBEDDING.fasttext_embeddings_index_zh
                if self.metadata["language"] == "ZH"
                else GET_EMBEDDING.fasttext_embeddings_index_en
            )
