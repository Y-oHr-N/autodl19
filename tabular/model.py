...
class TabularEmbeddingNN(nn.Module):
    def __init__(
        ...
        self.multi_head_attention = MultiHeadAttention(self.no_of_embs + self.no_of_numerical, 1)
    ...
    def forward(self, numerical_data, cat_data):
        ...
        X = self.multi_head_attention(
            X.reshape(batch_size, 1, feature_size),
            X.reshape(batch_size, 1, feature_size),
            X.reshape(batch_size, 1, feature_size)
        )
        X = X.reshape(batch_size, feature_size)
...
class MultiHeadAttention(nn.Module):
    ...
    def _reshape_to_batches(self, x):
        ...
        return x.reshape(batch_size, seq_len, self.head_num, sub_dim)\
                .permute(0, 2, 1, 3)\
                .reshape(batch_size * self.head_num, sub_dim, seq_len)
