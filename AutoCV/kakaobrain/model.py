...
class Model(LogicModel):
    ...
    def update_model(self):
        ...
        if self.is_multiclass():
            ...
            if self.is_video():
                self.model.loss_fn = skeleton.nn.BinaryCrossEntropyLabelSmooth(
                    num_class, epsilon=epsilon * 1000, reduction="none"
                )
            else:
                self.model.loss_fn = torch.nn.BCEWithLogitsLoss(
                    reduction="none"
                )
            self.tau = 8.0
            LOGGER.info(
                "[update_model] %s (tau:%f, epsilon:%f)",
                self.model.loss_fn.__class__.__name__,
                self.tau,
                epsilon,
            )
        else:
    ...
    def init_opt(self):
        ...
        params_fc = [
            ...
            if p.requires_grad and "fc" == n[:2] or "conv1d" == n[:6]
        ]
        ...
        LOGGER.info(
            ...
            self.optimizer._optimizer.__class__.__name__,
            batch_size,
        )
    ...
    def adapt(self, remaining_time_budget=None):
        ...
        if (
            self.hyper_params["conditions"]["use_fast_auto_aug"]
            and (train_score > 0.995 or self.info["terminate"])
            and remaining_time_budget > 120
            and valid_score > 0.01
            and self.dataloaders["valid"] is not None
            and not self.update_transforms
        ):
            LOGGER.info("[adapt] search fast auto aug policy")
            self.update_transforms = True
            self.info["terminate"] = True

            original_valid_policy = self.dataloaders[
                "valid"
            ...
            for policy_search in range(num_policy_search):
                selected_idx = np.random.choice(
                    list(range(len(policy))), num_sub_policy
                )
                selected_policy = [policy[i] for i in selected_idx]

                self.dataloaders["valid"].dataset.transform.transforms = (
                    original_valid_policy
                    + [
                ...
                searched_policy.append(
                    {"loss": loss, "score": score, "policy": selected_policy}
                )
            ...
            self.dataloaders[
                "valid"
            ].dataset.transform.transforms = original_valid_policy
    ...
    def epoch_train(self, epoch, train, model=None, optimizer=None):
        ...
        for step, (examples, labels) in enumerate(train):
            ...
            score = (
                auc
                if self.hyper_params["conditions"]["score_type"] == "auc"
                else float(nbac.detach().float())
            )
            metrics.append(
                {"loss": loss.detach().float().cpu(), "score": score,}
            )
        ...
        with torch.no_grad():
            for step, (examples, labels) in enumerate(valid):
                ...
                logits, loss = self.model(
                    examples, labels, tau=tau, reduction=reduction
                )
    ...
    def prediction(
        ...
        if model is None:
            ...
            LOGGER.info(
                "best checkpoints at %d/%d (valid loss:%f score:%f) tau:%f",
                best_idx + 1,
