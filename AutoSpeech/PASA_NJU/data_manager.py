    ...
    def get_train_data(
        ...
        if self._val_x is None:
            (
                ...
                self._train_y,
                self._val_y,
            ) = self._train_test_split(ratio=0.8)
        ...
        if use_new_train:
            ...
            if model_num > 0:
                self._lr_train_x = (
                    self._lr_train_y
                ) = self._lr_val_x = self._lr_val_y = None
            self._pre_train_x = (
                self._pre_train_y
            ) = self._pre_val_x = self._pre_val_y = None
            self.raw_max_length = None
            self.fea_max_length = None
            ...
            if len(train_samples) > 0:
                ...
                if self._lr_train_x is None:
                    ...
                    self._lr_train_y = np.concatenate(
                        (self._lr_train_y, train_y), axis=0
                    ).astype(np.int)
            if self._lr_val_x is None:
                self._lr_val_x = self.lr_preprocess(self._val_x)
                self._lr_val_y = self._val_y
            return (
                np.asarray(self._lr_train_x),
                np.asarray(self._lr_train_y),
                np.asarray(self._lr_val_x),
        ...
        return (
            np.asarray(self._pre_train_x),
            np.asarray(self._pre_train_y),
            np.asarray(self._pre_val_x),
            np.asarray(self._pre_val_y),
        )
    ...
    def nn_preprocess(self, x, n_mfcc=96, max_duration=5, is_mfcc=True):
        if self.raw_max_length is None:
            ...
            self.raw_max_length = min(
                max_duration * AUDIO_SAMPLE_RATE, self.raw_max_length
            )
            self.raw_max_length = max(
                MAX_AUDIO_DURATION * AUDIO_SAMPLE_RATE, self.raw_max_length
            )
